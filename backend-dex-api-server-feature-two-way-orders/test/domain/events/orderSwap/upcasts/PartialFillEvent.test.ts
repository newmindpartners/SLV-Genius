...
test('Upcasting of ORDER_SWAP__PARTIAL_FILL_ONCHAIN__SUCCESS V1 to latest is correct', () => {
  const oldVersionEvent = createMock<OrderSwapPartialFillOnChainSuccessEventV1>(
    {
      eventType: EventTypes.ORDER_SWAP__PARTIAL_FILL_ONCHAIN__SUCCESS,
      eventVersion: 1,
      eventPayload: {
  ...
  const latestVersionEvent =
    createMock<OrderSwapPartialFillOnChainSuccessEvent>({
      eventType: EventTypes.ORDER_SWAP__PARTIAL_FILL_ONCHAIN__SUCCESS,
      eventVersion: 2,
      eventPayload: {
  ...
  const upcastedOldVersion = upcastOrderSwapEvent(
    oldVersionEvent
  ) as OrderSwapPartialFillOnChainSuccessEvent;
  const upcastedLatestVersion = upcastOrderSwapEvent(
    latestVersionEvent
  ) as OrderSwapPartialFillOnChainSuccessEvent;
  ...
  const isUpcastingCorrect =
     ...
     */
    isOrderSwapPartialFillOnChainSuccessEvent(upcastedOldVersion) &&
    isOrderSwapPartialFillOnChainSuccessEvent(upcastedLatestVersion);
